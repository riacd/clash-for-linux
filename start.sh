...
# 修改端口配置 - 使用 mixed-port 避免端口冲突
sed -i 's/^port: .*/port: 7894/' $Conf_Dir/config.yaml
sed -i 's/^socks-port: .*/socks-port: 7895/' $Conf_Dir/config.yaml
sed -i 's/^redir-port: .*/redir-port: 7896/' $Conf_Dir/config.yaml

# 添加 mixed-port 如果不存在
if ! grep -q "^mixed-port:" $Conf_Dir/config.yaml; then
    sed -i '/^redir-port:/a mixed-port: 7893' $Conf_Dir/config.yaml
else
    sed -i 's/^mixed-port: .*/mixed-port: 7893/' $Conf_Dir/config.yaml
fi
...
echo -e "自定义配置应用完成！"
echo -e "  - Mixed Port: 7893 (推荐使用)"
echo -e "  - HTTP Port: 7894"
echo -e "  - SOCKS5 Port: 7895"
echo -e "  - API Port: 9091"
...
# 代理函数已在 ~/.bashrc 中预配置，无需重复添加
# 检查代理函数是否存在
if ! grep -q "proxy_on()" ~/.bashrc; then
    echo "⚠️  警告：~/.bashrc 中未找到代理函数，请手动配置"
    echo "建议运行：source ~/.bashrc"
fi
echo -e "     く__,.ヘヽ.        /  ,ー､ 〉"
echo -e "           ＼ ', !-─‐-i  /  /´"
echo -e "          ／｀ｰ'       L/／｀ヽ､"
