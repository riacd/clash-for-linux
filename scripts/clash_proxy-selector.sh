...
# Clash API地址
api_url="https://fba02.fbsubcn01.cc:2096/flydsubal/frghj7igkpr0luof?sub=2&extend=1"
Secret="6a46f5f7598be00fe0937113bbd567e026e4d9d71fc31191acf35dcb858a21e4"

# 获取Clash代理节点列表
get_proxy_list() {
    # 使用您的命令获取代理节点列表 (用 Python 解析 JSON 替代 jq)
    proxies=$(curl -s -XGET \
        -H "Content-Type: application/json" \
        -H "Authorization: Bearer ${Secret}" \
        $api_url/proxies \
        | python3 -c "import sys, json; data=json.load(sys.stdin); print('\n'.join(data['proxies']['Proxy']['all']))")
}
